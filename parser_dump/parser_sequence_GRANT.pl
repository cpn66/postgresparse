...
#REVOKE ALL ON SEQUENCE mem_trid_seq FROM PUBLIC;
#REVOKE ALL ON SEQUENCE mem_trid_seq FROM stbsadmin;
#GRANT SELECT,UPDATE ON SEQUENCE mem_trid_seq TO stbsadmin WITH GRANT OPTION;
#GRANT SELECT,UPDATE ON SEQUENCE mem_trid_seq TO stbsuser;

$p1='^s*(GRANT|REVOKE)\s+(\w+,?\w*)\s+ON\s+SEQUENCE (\w+)';
$dir=$ARGV[0]."/";
my $filestru=$ARGV[1];
